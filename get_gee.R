...
get_gee <- function(region, shape = "square", buffer.width = 1000, max.error = 1, scale = 30.922080775909325, max.pixels = 1e20,
  ...
  # Checks
  if(is.null(dem) & is.null(rgb)){
    stop("Need to provide Earth Engine Snippet to an image or image collection representing elevation and/or an image or image collection representing RGB values")  
  }
  ...
  }else if(!is.null(rgb) & is.null(dem)){
    latlng <- ee$Image$pixelLonLat()$addBands(RGB)
  }else if(!is.null(dem) & !is.null(rgb)){
  ...
  # Convert to arrays and raster/rasterBrick
